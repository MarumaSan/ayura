...
export default function MealPlanPage() {
    ...
    const handleConfirmPurchase = () => {
        ...
        const orderData = {
            ...
            plan: duration,
            price: duration === 'weekly' ? selectedSet!.priceWeekly : selectedSet!.priceMonthly,
        };
    ...
    const price = selectedSet
        ? (duration === 'weekly' ? selectedSet.priceWeekly : selectedSet.priceMonthly)
        : 0;

    const monthlySavings = selectedSet
        ? Math.round((selectedSet.priceWeekly * 4) - selectedSet.priceMonthly)
        : 0;

    const macros = selectedSet?.avgNutrition ? [
        { label: 'โปรตีน', val: selectedSet.avgNutrition.protein, max: 200, bgColor: 'bg-red-50', bar: 'from-red-400 to-rose-500', text: 'text-red-600' },
        { label: 'คาร์โบไฮเดรต', val: selectedSet.avgNutrition.carbs, max: 300, bgColor: 'bg-amber-50', bar: 'from-amber-400 to-yellow-500', text: 'text-amber-600' },
        { label: 'ไขมัน', val: selectedSet.avgNutrition.fat, max: 100, bgColor: 'bg-green-50', bar: 'from-green-400 to-emerald-500', text: 'text-green-600' },
    ] : [];
        ...
        <div className="min-h-screen pt-24 pb-16 bg-[var(--color-bg-section)]">
            <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="flex items-center gap-3 mb-8">
                    <div className={`flex items-center justify-center w-8 h-8 rounded-full text-sm font-bold transition-all ${step >= 1 ? 'bg-[var(--color-primary)] text-white' : 'bg-gray-200 text-gray-400'}`}>1</div>
                    <div className="text-sm font-medium text-[var(--color-text-light)]">เลือกเซ็ต</div>
                    <div className={`flex-1 h-0.5 rounded-full transition-all ${step === 2 ? 'bg-[var(--color-primary)]' : 'bg-gray-200'}`} />
                    <div className={`flex items-center justify-center w-8 h-8 rounded-full text-sm font-bold transition-all ${step === 2 ? 'bg-[var(--color-primary)] text-white' : 'bg-gray-200 text-gray-400'}`}>2</div>
                    <div className="text-sm font-medium text-[var(--color-text-light)]">เลือกระยะเวลา</div>
                </div>
                ...
                {step === 2 && selectedSet && (
                    <div className="animate-fade-in">
                        ...
                        <div className="mb-8">
                            ...
                            <p className="text-[var(--color-text-light)]">เซ็ต <strong>{selectedSet.name}</strong> — เลือกว่าจะสมัครแค่สัปดาห์เดียวหรือเหมาจ่ายรายเดือน</p>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                            ...
                            >
                                <div className="flex justify-between items-start mb-3">
                                    ...
                                    <div className="text-right">
                                        <div className="text-2xl font-bold text-[var(--color-primary)]">฿{selectedSet.priceWeekly.toLocaleString()}</div>
                            ...
                            >
                                ...
                                <div className="flex justify-between items-start mb-3">
                                    ...
                                    <div className="text-right">
                                        <div className="text-2xl font-bold text-[var(--color-primary)]">฿{selectedSet.priceMonthly.toLocaleString()}</div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            ...
                            <div className="lg:col-span-2 glass-card p-6">
                                <h4 className="font-bold text-[var(--color-primary-dark)] mb-5 flex items-center gap-2">
                                    📊 สารอาหารเฉลี่ยต่อวัน
                                </h4>

                                {selectedSet.avgNutrition ? (
                                    <>
                                        {/* Calorie hero */}
                                        <div className="bg-gradient-to-r from-orange-50 to-amber-50 rounded-2xl p-5 border border-orange-100 text-center mb-4">
                                            <div className="text-4xl font-bold text-orange-500 mb-1">
                                                {selectedSet.avgNutrition.calories.toLocaleString()}
                                        ...
                                        <p className="text-xs text-center text-[var(--color-text-muted)] mt-4">
                                            * ค่าโดยประมาณจากเมนูอาหารที่แนะนำในเซ็ตนี้
                                        </p>
                                    </>
                                ) : (
                            ...
                            <div className="lg:col-span-1">
                                <div className="glass-card p-6 sticky top-28">
                                    ...
                                    <p className="text-xs text-center text-[var(--color-text-muted)] mt-3">
                                        🚚 จัดส่งฟรีทั่วไทย · ✅ วัตถุดิบออร์แกนิก
                                    </p>
                                </div>
                            </div>
                        </div>
